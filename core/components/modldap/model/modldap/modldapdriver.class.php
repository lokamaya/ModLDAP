...
class modLDAPDriver {
    ...
    const OPT_LDAP_PROTOCOL_VERSION = 'modldap.ldap_opt_protocol_version';
    const OPT_LDAP_TIMELIMIT = 'modldap.ldap_opt_timelimit';
    const OPT_LDAP_REFERRALS = 'modldap.ldap_opt_referrals';
    const OPT_LDAP_SSL_PORT = 'modldap.ldap_opt_ssl_port';
    ...
    */
    ...
    function __construct(modX $modx, array $config = array()) {   
        $this->modx =& $modx;
        $this->modx->log(modX::LOG_LEVEL_DEBUG, 'modLDAPDriver Class');
        
        $this->config = array_merge(array(
        ), $config);

        if ($this->checkLdapSupport() === false) {
            $this->modx->log(modX::LOG_LEVEL_ERROR,'[ModLDAP] No LDAP support for PHP. See: http://www.php.net/ldap');
        }
    ...
    */
    public function connect() {
        ...
        $this->modx->log(modX::LOG_LEVEL_DEBUG, $dc);
        
        if ($useSsl) {
            $this->_conn = ldap_connect("ldaps://".$dc, (int)$this->getOption(modLDAPDriver::OPT_LDAP_SSL_PORT, 636));
        } else {
        ...
        $username = $this->getOption(modLDAPDriver::OPT_ADMIN_USERNAME, '');
        $password = $this->getOption(modLDAPDriver::OPT_ADMIN_PASSWORD, '');
        ...
        $baseDn = $this->getOption(modLDAPDriver::OPT_BASE_DN, '');

        if (empty($baseDn)) {
            $this->setOption(modLDAPDriver::OPT_BASE_DN, $this->findBaseDn());
        }
    ...
    */
    ...
    public function authenticate($username, $password, $preventRebind = false){
        ...
        // Bind as the user
        $accountSuffix = $this->getOption(modLDAPDriver::OPT_ACCOUNT_SUFFIX, '@forest.local');
        ...
        $adminPassword = $this->getOption(modLDAPDriver::OPT_ADMIN_PASSWORD, '');
        $adminUsername = $this->getOption(modLDAPDriver::OPT_ADMIN_USERNAME, '');
        ...
        if (!empty($adminPassword) && !$preventRebind) {
            ...
            if (!$this->_bind) {
                ...
                $this->modx->log(modX::LOG_LEVEL_ERROR, '[ModLDAP] Rebind to Active Directory failed. AD said: ' . $this->getLastError());
    ...
    */
    public function userInfo($username, array $fields = array(), $isGUID = false){
        ...
        if ($entries[0]['count'] >= 1) {
            ...
            if ($this->getOption(modLDAPDriver::OPT_REAL_PRIMARYGROUP) && isset($entries[0]["primarygroupid"][0]) && isset($entries[0]["objectsid"][0])) {
